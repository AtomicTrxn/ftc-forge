...
- **No real Gradle build exists yet.** This environment has JDK 17 (Temurin) but no Gradle/Maven installed. Everything was compiled and run directly via `javac`/`java`. This is a real gap against R3's assumption of a proper build — a follow-up should set up the actual Gradle multi-module build R2/R3 describe (JDK 21 target, `jlink` image with `jdk.compiler`) so this stops depending on manual classpath assembly.
...
2. **Real Gradle build setup** is now the most valuable infrastructure gap — needed both to close the dependency-resolution deviation above and to get off ad hoc `javac` classpath assembly before Phase 2 adds jMonkeyEngine as a real dependency.
