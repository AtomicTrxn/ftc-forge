...
3. Implement the executor using the classloading mechanism from R3: point it at a folder containing a `sim.config` (per R3's format), compile/load the OpMode class, and drive its lifecycle on a simple fixed-timestep loop. Use R3's parent/child classloader split (parent exposes only the mock SDK + JDK; child reloads team code + resolved third-party JARs fresh each time) so simulator internals stay invisible to team code and static state resets cleanly on reload. Target JDK 21 LTS per R2, with a `jlink` image that includes `jdk.compiler`. Per R3's simulated-time decision, `ElapsedTime` reads the simulator's own timer but v1 otherwise runs in real time — no pause/fast-forward — so this doesn't require any clock-virtualization work in Phase 1.
